...
test('Solidly adapter reads actual stable/fee metadata and routes it through the matching ABI', async () => {
  ...
  const client = {
    readContract: async (request: { functionName: string; args?: readonly unknown[]; blockNumber?: bigint }) => {
      ...
      if (request.functionName === 'fee') return 4n;
  ...
  const quote = await quoteExactInput(client, {
    venue: { kind: 'aerodrome', label: 'test', router: ROUTER, factory: FACTORY, pool: POOL, feeBps: 30 },
  ...
  assert.deepEqual(calls.map((call) => call.blockNumber), [456n, 456n, 456n]);
});
